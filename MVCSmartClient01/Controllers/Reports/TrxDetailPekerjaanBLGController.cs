...
namespace MVCSmartClient01.Controllers
{
    ...
    public class TrxDetailPekerjaanBLGController : Controller
    {
        HttpClient client;
        ...
        public TrxDetailPekerjaanBLGController()
        {
        ...
        //The DELETE method
        [HttpPost]
        public async Task<ActionResult> _Delete(int IdData, trxDetailPekerjaan Emp)
        {
            HttpResponseMessage responseMessage = await client.DeleteAsync(url + "/" + IdData);
            if (responseMessage.IsSuccessStatusCode)
            {
