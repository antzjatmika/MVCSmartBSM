...
namespace MVCSmartClient01.Controllers
{
    ...
    public class TrxTenagaAhliController : Controller
    {
        ...
        public async Task<ActionResult> GetByIdHeader(int IdHeader)
        {
            HttpResponseMessage responseMessage = await client.GetAsync(url + "/GetByIdHeader/" + IdHeader.ToString());
            if (responseMessage.IsSuccessStatusCode)
            {
                var responseData = responseMessage.Content.ReadAsStringAsync().Result;
                var myData = JsonConvert.DeserializeObject<List<trxTenagaAhli>>(responseData);
        ...
        public ActionResult _GetByIdHeader(int IdHeader)
        {
            //string conn = ConfigurationManager.ConnectionStrings["DB_Connect"].ConnectionString;

            HttpResponseMessage responseMessage = client.GetAsync(url + "/GetByIdHeader/" + IdHeader.ToString()).Result;
            if (responseMessage.IsSuccessStatusCode)
            {
                var responseData = responseMessage.Content.ReadAsStringAsync().Result;
                var myData = JsonConvert.DeserializeObject<List<trxTenagaAhli>>(responseData);
        ...
        private bool ImportXLS(Guid pgdPointer, int pintBarisMulai, int pintKolomMulai, string pstrFileName, string pstrFileExtension)
        {
            ...
            {
                string fileLocation = Server.MapPath("~/Content/FileUpload/") + pstrFileName;
        ...
        public ActionResult _PopulateTAFromFile(HttpPostedFileBase file)
        {
            ...
            if (Request.Files["file"].ContentLength > 0)
            {
                ...
                for (int i = intBarisMulaiData; i < ds.Tables[0].Rows.Count; i++)
                {
                    string conn = ConfigurationManager.ConnectionStrings["DB_Connect"].ConnectionString;
                    SqlConnection con = new SqlConnection(conn);
                    ...
                    SqlCommand cmd = new SqlCommand(queryByParam, con);
                    cmd.Parameters.AddWithValue("@Value1", 0);
                    cmd.Parameters.AddWithValue("@Value2", gdPointer.ToString());
                    cmd.Parameters.AddWithValue("@Value3", ds.Tables[0].Rows[i][intKolomMulaiData].ToString());
                    cmd.Parameters.AddWithValue("@Value4", ds.Tables[0].Rows[i][intKolomMulaiData + 1].ToString());
                    cmd.Parameters.AddWithValue("@Value5", ds.Tables[0].Rows[i][intKolomMulaiData + 2].ToString());

                    cmd.Parameters.AddWithValue("@Value6", ds.Tables[0].Rows[i][intKolomMulaiData + 3].ToString());
                    cmd.Parameters.AddWithValue("@Value7", ds.Tables[0].Rows[i][intKolomMulaiData + 4].ToString());
                    cmd.Parameters.AddWithValue("@Value8", ds.Tables[0].Rows[i][intKolomMulaiData + 5].ToString());
                    cmd.Parameters.AddWithValue("@Value9", ds.Tables[0].Rows[i][intKolomMulaiData + 6].ToString());
                    cmd.Parameters.AddWithValue("@Value10", ds.Tables[0].Rows[i][intKolomMulaiData + 7].ToString());

                    cmd.Parameters.AddWithValue("@Value11", ds.Tables[0].Rows[i][intKolomMulaiData + 8].ToString());
                    cmd.Parameters.AddWithValue("@Value12", ds.Tables[0].Rows[i][intKolomMulaiData + 9].ToString());
                    cmd.Parameters.AddWithValue("@Value13", ds.Tables[0].Rows[i][intKolomMulaiData + 10].ToString());
                    cmd.Parameters.AddWithValue("@Value14", "Catatan Kecil 1234 5 4321");
                    cmd.Parameters.AddWithValue("@Value15", "Admin");
